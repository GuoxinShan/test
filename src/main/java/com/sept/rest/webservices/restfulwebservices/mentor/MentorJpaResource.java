...
public class MentorJpaResource {
	...
	//set the new rating and number of mentee taught for mentor
	@PutMapping("/jpa/users/{mentorname}/mentors/{mentorId}/setRate/{rate}")
	public void setRate(@PathVariable String mentorName,@PathVariable long mentorId,@PathVariable double rate) {
		
				mentorJpaRepository.findById(mentorId).get().setUsername(mentorName);
				mentorJpaRepository.findById(mentorId).get().addRate(rate);
				mentorJpaRepository.findById(mentorId).get().addNumOfMentee();	
//				Mentor temp = new Mentor(mentor.getId(),mentorName, courseId, mentor.getCourseName(), mentor.getMentorStartDate(), mentor.getTotalRate(), mentor.getNumOfMentee());
//				Mentor mentorUpdated = mentorJpaRepository.save(temp);
//				new ResponseEntity<Mentor>(temp, HttpStatus.OK);
	}
}
